...
##calculate_mean_relative_gain_instruction
def calculate_mean_relative_gain_instruction(all_result_dict, model_list):
    all_result_list = []
    for model, model_result in all_result_dict.items():
        model_result_dict =  model_result
        model_result_list = []
        # print(f'the model is {model}, model_result_dict is {model_result_dict}\n')
        for key, value in model_result_dict.items():
            model_result_list.append(value)
        all_result_list.append(model_result_list)   
    all_result_list = np.array(all_result_list)
    # print(f'the shape of all_result_list is {np.shape(all_result_list)}') ##(20, 9, 10) --> (num_model, num_task, num_instrutions)
    
    all_mean_relative_gain_instruction = []
    instruction_mean = np.mean(all_result_list, axis=-1)
    instruction_mean += 1e-6
    # print(f'the shape of instruction_mean is {np.shape(instruction_mean)}') ##(20, 9)
    num_model, num_task, num_instrutions = np.shape(all_result_list)
    for i in range(num_instrutions):
        model_result = all_result_list [:, :, i] ##(20, 9)
        mean_relative_gain = 100 * (model_result - instruction_mean) / instruction_mean
        all_mean_relative_gain_instruction.append(mean_relative_gain) ##(10, 20, 9)
    all_mean_relative_gain_instruction = np.array(all_mean_relative_gain_instruction)
    # print(f'the shape of all_mean_relative_gain_instruction is{np.shape(all_mean_relative_gain_instruction)}')
    instruction_mean_relative_gain = np.average(all_mean_relative_gain_instruction, axis=1)
    # instruction_mean_relative_gain = instruction_mean_relative_gain.T
    for i in range(len(instruction_mean_relative_gain)):
        for j in range(len(instruction_mean_relative_gain[i])):
            instruction_mean_relative_gain[i][j] = round(instruction_mean_relative_gain[i][j], 2)
        
    print(np.shape(instruction_mean_relative_gain)) ##(10, 9)
    print(instruction_mean_relative_gain)
    return instruction_mean_relative_gain
        

##calculate_mean_relative_gain_model
def calculate_mean_relative_gain_model(all_result_dict, model_list):
    np.set_printoptions(suppress=True,)
    all_result_list = []
    for model, model_result in all_result_dict.items():
        model_result_dict =  model_result
    ...
    all_mean_relative_gain_model = []
    model_mean = np.mean(all_result_list, axis=0)
    model_mean += 1e-6
    # print(f'the shape of model_mean is {np.shape(model_mean)}') ##(9, 10)
    # print(all_result_list[:, -1, :])
    # print(model_mean[-1, :])
    num_model, num_task, num_instrutions = np.shape(all_result_list)
    for i in range(num_model):
        model_result = all_result_list [i, :, :] ## 
        mean_relative_gain = 100 * (model_result - model_mean) / model_mean ##(9, 10)
        all_mean_relative_gain_model.append(mean_relative_gain) 
    all_mean_relative_gain_model = np.array(all_mean_relative_gain_model)
    # print(all_mean_relative_gain_model[:, -1, :])
    # print(f'the shape of all_mean_relative_gain_model is{np.shape(all_mean_relative_gain_model)}') ##(20, 9, 10)
    
    model_mean_relative_gain = np.average(all_mean_relative_gain_model, axis=-1) ##(20, 9)
    # instruction_mean_relative_gain = instruction_mean_relative_gain.T
    for i in range(len(model_mean_relative_gain)):
        for j in range(len(model_mean_relative_gain[i])):
            model_mean_relative_gain[i][j] = round(model_mean_relative_gain[i][j], 2)
        
    print(np.shape(model_mean_relative_gain))
    print(model_mean_relative_gain)
    return model_mean_relative_gain
...
if __name__ == "__main__":
    ...
    instruction_mean_relative_gain = calculate_mean_relative_gain_instruction(all_result_dict, model_list)
    model_mean_relative_gain = calculate_mean_relative_gain_model(all_result_dict, model_list)
