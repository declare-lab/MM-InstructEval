...
def get_best_acc(model_list, dataset_list, model_result_dict):
    all_best_all_list =np.zeros((len(model_list), len(dataset_list)))
    print(np.shape(all_best_all_list))
    for i, model in enumerate(model_list):
        for j, dataset in enumerate(dataset_list):
            model_acc_dict  = model_result_dict[model]
            dataset_acc_list = model_acc_dict[dataset]
            # print(dataset_acc_list, np.shape(dataset_acc_list))
            max_acc = np.max(dataset_acc_list)
            
            max_acc_index = dataset_acc_list.index(max_acc)+1
            max_acc = round(max_acc, 2)
            all_best_all_list[i][j] = max_acc
            
            print(f'The {dataset} dataset on {model} model, the best acc and index is {max_acc}_{max_acc_index}')
    total_best_acc = np.sum(all_best_all_list, axis=1)
    print(all_best_all_list)
    print(total_best_acc)
    return all_best_all_list
...
if __name__ == "__main__":
    ...
    model_list = ['chatGPT', 'LLaMA1_7B', 'LLaMA1_13B', 'LLaMA2_7B', 'LLaMA2_13B', 'flanT5', 
                  'openFlamingo', 'Fromage', 'LLaVA_7B', 'LLaVA_13B',  'MiniGPT4', 'mPLUG_OWL', 
                  'Adapterv2',
                  'VPGTrans', 'MultiGPT',
    ...
    # get_best_acc(model_list, dataset_list, all_result_dict)    
    # ss = [69.41, 56.55, 53.18, 48.17, 65.48, 59.97, 72.7,  60.84, 69.02, 38.28]
    
    ScienceQA_dataset_list = ['ScienceQA', 'ScienceQA_no_context']
    ScienceQA_model_list = ['openFlamingo', 'Fromage', 'LLaVA_7B', 'LLaVA_13B',  'MiniGPT4', 'mPLUG_OWL', 
                  'Adapterv2',
                  'VPGTrans', 'MultiGPT',
                  'LaVIN_7B', 'LaVIN_13B', 'Lynx',
                  'BLIP_2', 'InstructBLIP']
    all_best_all_list = get_best_acc(model_list, dataset_list, model_result_dict=all_result_dict)   
    
    print(np.shape(all_best_all_list))
    for i in range(len(all_best_all_list)):
        
        Difference = round(all_best_all_list[i][0] - all_best_all_list[i][1], 2)
        print(Difference)
