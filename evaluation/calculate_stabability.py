...
def calculate_stabability_for_model(all_result_dict, model_list):
    ...
    model_std = np.std(all_result_list, axis=-1)
    for i in range(len(model_std)):
        for j in range(len(model_std[i])):
            model_std[i][j] = round(model_std[i][j], 2)
    print(np.shape(model_std))
    print(model_std)
    
    instruction_std = np.std(all_result_list, axis=0).T
    for i in range(len(instruction_std)):
        for j in range(len(instruction_std[i])):
            instruction_std[i][j] = round(instruction_std[i][j], 2)
    print(np.shape(instruction_std))
    print(instruction_std)
...
if __name__ == "__main__":
    ...
    MLLM_model_list = [
                  ...
                  'LaVIN_7B', 'LaVIN_13B', 'Lynx',
                  'BLIP_2', 'InstructBLIP']
    calculate_stabability_for_model(all_result_dict, model_list)