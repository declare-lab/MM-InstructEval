...
from multimodal_eval_main.models.fromage import utils
    ...
    def generate(
        ...
        with torch.no_grad():  # no tracking history
            ...
            for i in range(max_len):
                ...
                if labels is not None:
                    temp = torch.full_like(logits, fill_value=-1e9)
                    for x in labels:
                        i = self.tokenizer(x, add_special_tokens=False).input_ids[0]
                        temp[:, i] = logits[:, i]
                    logits = temp
                    min_word_tokens = 9999  # Only generate labels and no retrieval
...
class Fromage(nn.Module):
    ...
    def generate_for_images_and_texts(
        ...
        bs, length, dim = input_embs.shape

        if max_input_length is not None and length > max_input_length:
            input_embs = input_embs[:, slice(-max_input_length, length), :]
            input_ids = input_ids[:, slice(-max_input_length + 1, length)]
