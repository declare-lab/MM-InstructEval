...
class Blip2T5Instruct(Blip2Base):
    ...
    def __init__(
        ...
        self.t5_model = T5ForConditionalGeneration.from_pretrained(
            t5_model, config=t5_config, load_in_8bit=True, device_map="auto"
        )
    ...
    def forward(self, samples):
        ...
        if "few_shot_samples" in samples.keys():
        ...
        with self.maybe_autocast(dtype=torch.float16):
            ...
            return {"loss": loss, 'logits': logits, 'input_ids': input_tokens.input_ids}
    ...
    def prepare_few_shot_embeds(self, samples):
        ...
        # )[0]
        this_n_fs=self.num_few_shot_examples
        if this_n_fs == 0:
            return None, None
        ...
        for n in range(this_n_fs):
            ...
            # text_input.append(sample["text"][n])
            '''for me'''
            images.append(samples[n]["image"])
            text_input.append(samples[n]["text"])
    ...
    @torch.no_grad()
    def generate_with_few_shot_samples(
        self,
        samples,
        use_nucleus_sampling=False,
        num_beams=5,
        ...
        #         samples["few_shot_samples"]
        #     )
        self.few_shot_prob=1
        self.num_few_shot_examples=len(samples["few_shot_samples"])
        ...
        with self.maybe_autocast(dtype=torch.float16):
            inputs_embeds = self.t5_model.encoder.embed_tokens(input_tokens.input_ids)
            # print('the fs_embeds.shape is {}'.format(fs_embeds.shape))
            # print('the before inputs_embeds.shape is {}'.format(inputs_embeds.shape))
            
            inputs_embeds = torch.cat([inputs_t5, inputs_embeds], dim=1)
            if fs_embeds is not None:
                inputs_embeds = torch.cat([fs_embeds, inputs_embeds], dim=1)
                encoder_atts = torch.cat([fs_atts, encoder_atts], dim=1)
            # print('the after inputs_embeds.shape is {}'.format(inputs_embeds.shape))
