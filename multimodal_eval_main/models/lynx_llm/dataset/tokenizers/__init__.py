...
def build_tokenizer(LLM: str, use_left_pad: bool):
    rpath = os.path.join("/data/xiaocui/weights/lynx_llm/", LLM)
    assert os.path.exists(rpath)

    num_new_tokens = 0

    if LLM in ['vicuna-7b']:
        from multimodal_eval_main.models.lynx_llm.models.llms.llama.tokenization_llama import LlamaTokenizer
