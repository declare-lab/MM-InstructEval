...
from multimodal_eval_main.models.lynx_llm.dataset.utils import sample_frame_ids

from multimodal_eval_main.models.lynx_llm.dataset import get_image_transform, get_video_transform
from multimodal_eval_main.models.lynx_llm.dataset.tokenizers import build_tokenizer
...
class LynxEvalDatasetBase(Dataset):
    ...
    def get_text_input(self, ann):
        ...
        if self.task=="MASC":
            aspect = ann["aspect"]
            ann['text'] = ann['text'].replace('$T$', aspect)
    ...
    def __getitem__(self, index):
        ...
        if self.task=='MASC':
            aspect = ann['aspect']
        if self.task == "QA":
            label = ann['answer_text']
            label_index = ann['answer_index']
            choices = ann['choices']
        else: 
            label = ann['label']
        ...
        if self.task != "QA":
            return idx, vision_input, input_ids, label
        else:
            return idx, vision_input, input_ids, ann['text'],  ann['image'], choices, label, label_index
    ...
    def collate_fn(self, batch):
        ...
        for x in zip(*batch):
            inputs.append(x)
        if self.task != "QA":
            idx, vision_input, input_ids, label = inputs
        else:
            idx, vision_input, input_ids, texts, images, choices, label, label_index = inputs
        ...
        if self.task != "QA":
            return idx, vision_input, input_ids_pad, input_atts_pad, label
        else:
            return idx, vision_input, input_ids_pad, input_atts_pad, texts, images, choices, label, label_index
