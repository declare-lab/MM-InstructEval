...
class LynxBase(nn.Module):
    ...
    def build_vision_encoder(self, config, freeze_params=True):
        ...
        if config['vision_encoder'] == 'eva_vit_1b':
            from multimodal_eval_main.models.lynx_llm.models.vits.eva_vit import create_eva_vit_g
    ...
    def build_LLM(self, config, freeze_params=True, num_new_tokens=0):
        ...
        if config['LLM'] in ['vicuna-7b']:
            from multimodal_eval_main.models.lynx_llm.models.llms.llama.modeling_llama import LlamaForCausalLM, LlamaConfig

            rpath = os.path.join("/data/xiaocui/weights/lynx_llm/", config['LLM'])
    ...
    def build_bridge(self, config, load_params=True):
        ...
        if config['bridge'] == 'resampler':
            from multimodal_eval_main.models.lynx_llm.models.bridges.resampler import PerceiverResampler
