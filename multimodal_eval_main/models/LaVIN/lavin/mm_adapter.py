...
from multimodal_eval_main.models.LaVIN import lavin
from typing import Optional, Tuple
from  torch.cuda.amp import autocast
from multimodal_eval_main.models.LaVIN.lavin import eval_model
...
def set_MMAdapter(model, method, dim=8, s=1, set_forward=True,t=10,gradient_checkpointing=False):
    if method == 'block':
        ...
        for _ in model.children():
            if type(_) == eval_model.TransformerBlock or type(_) == eval_model.TransformerBlock:
        ...
        for _ in model.children():
            if type(_) == lavin.model.TransformerBlock or type(_) == eval_model.TransformerBlock:
                _.adapter_attn = RepAdapter_Router(_.dim,hidden_dim=dim,scale=s,t=t).half()
                _.s = s
                _.t=t
                _.gradient_checkpointing = gradient_checkpointing
                if type(_) == eval_model.TransformerBlock:
...
from multimodal_eval_main.models.LaVIN.clip.model import ResidualAttentionBlock
